...
#define MATCH_RETURN_STRING(x) case x: return #x;
#define BOOL_TO_YES_NO(x) ((x) ? "yes" : "no")

static const char *
device_type_str(cl_device_type dt) {
    switch (dt) {
        MATCH_RETURN_STRING(CL_DEVICE_TYPE_GPU)
        MATCH_RETURN_STRING(CL_DEVICE_TYPE_CPU)
        MATCH_RETURN_STRING(CL_DEVICE_TYPE_ACCELERATOR)
    default:
        return NULL;
    }
}
...
err_str(cl_int err) {
    switch (err) {
        MATCH_RETURN_STRING(OCL_FILE_NOT_FOUND                )
        MATCH_RETURN_STRING(OCL_BAD_PLATFORM_IDX              )
        MATCH_RETURN_STRING(OCL_BAD_DEVICE_IDX                )
        MATCH_RETURN_STRING(CL_SUCCESS                        )
        MATCH_RETURN_STRING(CL_DEVICE_NOT_FOUND               )
        MATCH_RETURN_STRING(CL_DEVICE_NOT_AVAILABLE           )
        MATCH_RETURN_STRING(CL_COMPILER_NOT_AVAILABLE         )
        MATCH_RETURN_STRING(CL_MEM_OBJECT_ALLOCATION_FAILURE  )
        MATCH_RETURN_STRING(CL_OUT_OF_RESOURCES               )
        MATCH_RETURN_STRING(CL_OUT_OF_HOST_MEMORY             )
        MATCH_RETURN_STRING(CL_PROFILING_INFO_NOT_AVAILABLE   )
        MATCH_RETURN_STRING(CL_MEM_COPY_OVERLAP               )
        MATCH_RETURN_STRING(CL_IMAGE_FORMAT_MISMATCH          )
        MATCH_RETURN_STRING(CL_IMAGE_FORMAT_NOT_SUPPORTED     )
        MATCH_RETURN_STRING(CL_BUILD_PROGRAM_FAILURE          )
        MATCH_RETURN_STRING(CL_MAP_FAILURE                    )
        MATCH_RETURN_STRING(CL_MISALIGNED_SUB_BUFFER_OFFSET   )
        MATCH_RETURN_STRING(CL_COMPILE_PROGRAM_FAILURE        )
        MATCH_RETURN_STRING(CL_LINKER_NOT_AVAILABLE           )
        MATCH_RETURN_STRING(CL_LINK_PROGRAM_FAILURE           )
        MATCH_RETURN_STRING(CL_DEVICE_PARTITION_FAILED        )
        MATCH_RETURN_STRING(CL_KERNEL_ARG_INFO_NOT_AVAILABLE  )
        MATCH_RETURN_STRING(CL_INVALID_VALUE                  )
        MATCH_RETURN_STRING(CL_INVALID_DEVICE_TYPE            )
        MATCH_RETURN_STRING(CL_INVALID_PLATFORM               )
        MATCH_RETURN_STRING(CL_INVALID_DEVICE                 )
        MATCH_RETURN_STRING(CL_INVALID_CONTEXT                )
        MATCH_RETURN_STRING(CL_INVALID_QUEUE_PROPERTIES       )
        MATCH_RETURN_STRING(CL_INVALID_COMMAND_QUEUE          )
        MATCH_RETURN_STRING(CL_INVALID_HOST_PTR               )
        MATCH_RETURN_STRING(CL_INVALID_MEM_OBJECT             )
        MATCH_RETURN_STRING(CL_INVALID_IMAGE_FORMAT_DESCRIPTOR)
        MATCH_RETURN_STRING(CL_INVALID_IMAGE_SIZE             )
        MATCH_RETURN_STRING(CL_INVALID_SAMPLER                )
        MATCH_RETURN_STRING(CL_INVALID_BINARY                 )
        MATCH_RETURN_STRING(CL_INVALID_BUILD_OPTIONS          )
        MATCH_RETURN_STRING(CL_INVALID_PROGRAM                )
        MATCH_RETURN_STRING(CL_INVALID_PROGRAM_EXECUTABLE     )
        MATCH_RETURN_STRING(CL_INVALID_KERNEL_NAME            )
        MATCH_RETURN_STRING(CL_INVALID_KERNEL_DEFINITION      )
        MATCH_RETURN_STRING(CL_INVALID_KERNEL                 )
        MATCH_RETURN_STRING(CL_INVALID_ARG_INDEX              )
        MATCH_RETURN_STRING(CL_INVALID_ARG_VALUE              )
        MATCH_RETURN_STRING(CL_INVALID_ARG_SIZE               )
        MATCH_RETURN_STRING(CL_INVALID_KERNEL_ARGS            )
        MATCH_RETURN_STRING(CL_INVALID_WORK_DIMENSION         )
        MATCH_RETURN_STRING(CL_INVALID_WORK_GROUP_SIZE        )
        MATCH_RETURN_STRING(CL_INVALID_WORK_ITEM_SIZE         )
        MATCH_RETURN_STRING(CL_INVALID_GLOBAL_OFFSET          )
        MATCH_RETURN_STRING(CL_INVALID_EVENT_WAIT_LIST        )
        MATCH_RETURN_STRING(CL_INVALID_EVENT                  )
        MATCH_RETURN_STRING(CL_INVALID_OPERATION              )
        MATCH_RETURN_STRING(CL_INVALID_GL_OBJECT              )
        MATCH_RETURN_STRING(CL_INVALID_BUFFER_SIZE            )
        MATCH_RETURN_STRING(CL_INVALID_MIP_LEVEL              )
        MATCH_RETURN_STRING(CL_INVALID_GLOBAL_WORK_SIZE       )
        MATCH_RETURN_STRING(CL_INVALID_PROPERTY               )
        MATCH_RETURN_STRING(CL_INVALID_IMAGE_DESCRIPTOR       )
        MATCH_RETURN_STRING(CL_INVALID_COMPILER_OPTIONS       )
        MATCH_RETURN_STRING(CL_INVALID_LINKER_OPTIONS         )
        MATCH_RETURN_STRING(CL_INVALID_DEVICE_PARTITION_COUNT )
    default:
        return NULL;
    }
}
...
ocl_print_device_details(cl_device_id dev, pretty_printer *pp) {
    ...
    const char *type_str = device_type_str(dt);
